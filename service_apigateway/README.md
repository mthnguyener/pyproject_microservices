...
- `service_apigateway`: Project main script directory - additional apps are added here
...
Container: <user_name>_<project_name>_<service>
    - `mthnguyener_service_apigateway`
...
```
A directory called `service_apigateway` will be created where the 
command was executed. This `service_apigateway` directory will be 
...
3. The new project is created 1-level up from the current project root directory
    - if current project directory is `projects/service_apigateway` 
...
The service_apigateway project is capable of creating a Jupyter 
...
## Dependencies
Since the service_apigateway utilizes NVIDIA optimized Docker 
...
service_apigateway environment, and how to use them.
